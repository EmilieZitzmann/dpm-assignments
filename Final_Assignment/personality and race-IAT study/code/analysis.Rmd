...
ggplot(data_processed_after_exclusions, aes(x = mean_extroversion)) +
  geom_histogram(binwidth = 0.2,
                 boundary = 0,
                 fill = "#69b3a2",  #Adjust the color for filling the histogram bars 
                 color = "#404080",  # Adjust the color for the borders 
                 alpha = 0.8) + # Added transparecy in order to make the plot look a bit cleaner 
  xlab("Mean Score Distribution for Extroversion") +
  ylab("Frequency") +
  theme_minimal() +  
  ...
  theme(panel.grid.minor = element_blank(),
        axis.title = element_text(size = 12, face = "bold"),  #Axis title
        axis.text = element_text(size = 10),  # Axis text
        plot.title = element_text(hjust = 0.5, size = 14),  # Centered plot title
...
ggplot(data_processed_after_exclusions, aes(x = mean_conscient)) +
  geom_histogram(binwidth = 0.2,
                 ...
                 color = "#404080",  # Adjust the color for the borders 
                 alpha = 0.8) +  # Added transparecy in order to make the plot look a bit cleaner 
  xlab("Mean Score Distribution for Conscientiousness") +
  ylab("Frequency") +
  theme_minimal() +  
  scale_x_continuous(breaks = seq(1, 7, 0.5), #Location of thick marks specified: Code creates breaks every 0.5 increments
                     limits = c(1, 7), expand = c(0, 0)) +  # X-Axis is limited to a range of 1 to 7 (only values within                                                                this range are shown).Limits = 0 means x is shown as                                                                      specified
...
ggplot(data_processed_after_exclusions, aes(x = mean_neuroticism)) +
  geom_histogram(binwidth = 0.2,
                 ...
                 color = "#404080", 
                 alpha = 0.8) +   
  xlab("Mean Score Distribution for Neuroticism") +
  ylab("Frequency") +
  theme_minimal() +  
...
ggplot(data_processed_after_exclusions, aes(x = mean_agreeable)) +
  geom_histogram(binwidth = 0.2,
                 ...
                 color = "#404080", 
                 alpha = 0.8) +   
  xlab("Mean Score Distribution for Agreeableness") +
  ylab("Frequency") +
  theme_minimal() +  
  scale_x_continuous(breaks = seq(1, 7, 0.5), 
                     limits = c(1, 7), expand = c(0, 0)) +
...
ggplot(data_processed_after_exclusions, aes(x = mean_openness)) +
  geom_histogram(binwidth = 0.2,
                 ...
                 color = "#404080", 
                 alpha = 0.8) +   
  xlab("Mean Score Distribution for Openness") +
  ylab("Frequency") +
  theme_minimal() +  
  scale_x_continuous(breaks = seq(1, 7, 0.5), 
                     limits = c(1, 7), expand = c(0, 0)) +
...
# Create the histogram with the normal distribution lines
ggplot(data_processed_after_exclusions, aes(x = D)) +
  geom_histogram(binwidth = 0.08,
                 ...
                 color = "#404080",
                 alpha = 0.8) +
  xlab("Distribution of the IAT Cowald D Score") +
  ylab("Frequency") +
  theme_minimal() +
...
```{r}
# Calculate correlations and add them as new columns

bfi_iat_corr_matrix <- data_processed_after_exclusions |>
  select(D, 
         starts_with("mean_")) |>
  summarise(across(.cols = starts_with("mean_"), #specifies columns where the function is applied
                .fns = ~ cor(D, ., method = "pearson", use ="complete.obs"), #defining the function by tilde and what it should do. Calculated the corr between each selected mean function and the D. The dot represents the columns so each one is calculated. Across is used to apply to multiple columns and summarise because we want only one column and not several--> mutate does 164 columns  
                .names= "cor_{.col}")) |> #naming pattern for the new columns 
  round(2) #rounds to two decimal places 


#With added p value of the correlation
bfi_iat_corr_matrix <- data_processed_after_exclusions |>
  select(D, starts_with("mean_")) |>
  summarise(across(.cols = starts_with("mean_"),
                .fns = ~ cor(D, ., use = "complete.obs"),
                .names = "cor_{.col}"),
         across(.cols = starts_with("mean_"),
                .fns = ~ cor.test(D, ., method ="pearson", use = "complete.obs")$p.value,
                .names = "p_value_{.col}")) |>
  round(2) 
...
#Specify the information within the t test because otherwhise it produces a warning. The warning does not affect the calculation but still it is better to specify   

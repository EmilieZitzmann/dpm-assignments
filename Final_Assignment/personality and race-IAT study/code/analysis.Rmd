...
Distribution of the implicit association test is illustrated by the Greenwald D score. The score can range between -2 and +2. 
...
data_scatter_plot_extreme <- data_processed_after_exclusions |>
  mutate(extreme_score_IAT = ifelse(D <= quantile(D, 0.1) | D >= quantile(D, 0.9), "Extreme", "Normal"))

#Plot
plot_extroversion <- ggplot(data_scatter_plot_extreme, 
                            aes(y = D, x = mean_extroversion, 
                                color = extreme_score_IAT, 
                                shape = extreme_score_IAT)) +
  geom_jitter(data = filter(data_scatter_plot_extreme, extreme_score_IAT != "Extreme"),
              alpha = 0.7,
              width = 0.2, size = 2.5) +  
  geom_smooth(method = "lm",
              ...
              color = "#34495e",
              size = 0.8) +
  geom_point(data = filter(data_scatter_plot_extreme, extreme_score_IAT == "Extreme"),
             size = 2.2) +  # Adjust the size for the triangular points
  scale_color_manual(values = c("Normal" = "#2196F3", "Extreme" = "#673AB7"),
...
plot_conscient <- ggplot(data_scatter_plot_extreme, 
                         aes(y = D, x = mean_conscient, 
                             color = extreme_score_IAT, 
                             shape = extreme_score_IAT)) +
  geom_jitter(data = filter(data_scatter_plot_extreme, extreme_score_IAT != "Extreme"),
              alpha = 0.7,
...
```{r}
plot_neuroticism <- ggplot(data_scatter_plot_extreme, 
                         aes(y = D, x = mean_neuroticism, 
                             color = extreme_score_IAT, 
                             shape = extreme_score_IAT)) +
  geom_jitter(data = filter(data_scatter_plot_extreme, extreme_score_IAT != "Extreme"),
              alpha = 0.7,
...
plot_agreeable <- ggplot(data_scatter_plot_extreme, 
                         aes(y = D, x = mean_agreeable, 
                             color = extreme_score_IAT, 
                             shape = extreme_score_IAT)) +
  geom_jitter(data = filter(data_scatter_plot_extreme, extreme_score_IAT != "Extreme"),
              alpha = 0.7,
...
```{r}
plot_openness <- ggplot(data_scatter_plot_extreme, 
                         aes(y = D, x = mean_openness, 
                             color = extreme_score_IAT, 
                             shape = extreme_score_IAT)) +
  geom_jitter(data = filter(data_scatter_plot_extreme, extreme_score_IAT != "Extreme"),
              alpha = 0.7,
...
# Assuming you have individual facet plots: plot_extroversion, plot_conscient, plot_neuroticism, plot_agreeable, plot_openness

# Arrange the facet plots in a grid
facet_plots <- plot_grid(plot_extroversion, plot_agreeable, plot_conscient, plot_neuroticism, plot_openness, ncol = 2)

# Add a common title to the combined plot
combined_plot <- facet_plots +
  ggtitle("Regression Analysis: BFI Subscale Scores Predicting IAT Scores") +
  theme(plot.title = element_text(hjust = 0.5, size = 16, margin = margin(b = 20)))
...
combined_plot <- plot_extroversion + plot_conscient + plot_neuroticism + plot_agreeable + plot_openness

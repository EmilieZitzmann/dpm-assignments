...
Prior to this, it is necessary to inspect the raw dataset for duplicated or NA data within the 'unique_Id'. The resulting 'check_Id' dataframe reveals the presence of both duplicated and NA data, which is then extracted into a new dataframe.

In a subsequent step, missing and duplicated values are isolated. Any values not meeting the specified criteria are assigned as 'other/missing/error' for potential exclusion at a later stage, if required.

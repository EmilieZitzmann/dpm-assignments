...
Beforehand we have to check if there is duplicated or NA data within the unique_Id in the raw dataset. By creating the dataframe "check_Id" it becomes apparent that we have duplicated data and data with NA. These are extracted for the new dataframe. 
...
First it is checked, whether the bfi_dataset has any duplicated rows. as this is not the case we will continue by working with the raw bfi dataset directly.  
...
data_reversed_items <- data_raw_bfi |>
  mutate(across(all_of(vector_reversed_items), ~7 - .)) #"." represents column values that are subtracted                                                            from 7 (by performin ~7). This produces the                                                                 reverted items. 
...
witin the study each participant completed an Implicit Association Test assessing implicit racial evaluations between African-American and European-American people. Block 1, 2, 5 are practice blocks and must be discarded for analysis calculations. 
...
#making sure that trial_raction_time_in_ms is a numeric argument: 
...
2. An exclusion variable is created that sets participants with incomplete IAT data at trial level to          "exclude". The IAT should contain 120 trials in the relevant test blocks required for the analysis.
...
*Interpretation* : Some individuals have incompleted trial data and must therefore be set to "exclude"
...
3. Participants who have over 10% of their trials with reaction times less than 300                            milliseconds or an accuracy below 75% should be excluded. In order to do so, two variables that contain     the corresponding exclusion criteria are created. A proportion of these is then calculated so that          individuals can be identified for exclusion. 
...
data_iat_exclusion <- data_iat_corrected |>
  ...
  mutate(exclude_accuracy = ifelse(proportion_fast_reaction_time >= 0.10 | 
                                   proportion_low_accuracy_iat >= 0.75, 
                                   "exclude",
                                   "include"))
```

*Interpretation*: One individual should be excluded  
...
data_processed <- data_processed_before_exclusions |>
  mutate(exclude_participant = case_when(is.na(bfi_logical_number_violation) |
                                         ...
                                         TRUE ~ "include")) 
#Find out how many got filtered out 
...
*Interpretation*: 164 is not too bad as we have 179 participants in the datasets where the unique id is only counted once. so we excluded 15 participants. 

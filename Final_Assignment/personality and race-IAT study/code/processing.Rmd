...
data_iat_performance <- data_amp_raw %>%
  filter(blockcode %in% c(3, 4, 6, 7), trialcode != "instructions") %>%
  mutate(
    latency_prob = ifelse(latency < 100, TRUE, FALSE),
    accuracy_prob = ifelse(accuracy < 0.75, TRUE, FALSE)
  ) %>%
  group_by(subject) %>%
  summarize(
    proportion_fast_trials_amp = mean(latency_prob),
    proportion_low_accuracy_amp = mean(accuracy_prob)
  ) %>%
  mutate(
    exclude_amp_performance = ifelse(
      proportion_fast_trials_amp > 0.10 | proportion_low_accuracy_amp > 0.10,
      "exclude",
      "include"
    )
  )

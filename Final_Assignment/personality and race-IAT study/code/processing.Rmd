...
data_combined_exclusion <- dat_reversed_items |>
  ...
  mutate(completed_questionnaires = sum(!is.na(c(completed_q_e, 
                                                 ...
                                                 completed_q_a,
                                                 completed_q_o))),
         questionnaire_inclusion = case_when(completed_questionnaires >= 2 ~ 
                                             "include",
                                             TRUE ~ "exclude"))
...
data_completed_exclusion <- dat_reversed_items |>
  ...
  select(-contains("completed_q_"), 
         -"completed_questionnaires") #removing these for a clean dataset 



#Check 
data_completed_exclusion |>
  filter(questionnaire_inclusion == "exclude") |>
  select(unique_id) |>
  kable() |>
  kable_classic(full_width = FALSE)
...
data_mean_violation_bfi <- data_mean_bfi |>
  rowwise() |>
  mutate(score_violation = ifelse(all(c_across(contains("bfi")) #use all so it is checked for each row
...
#No mean violates the principle 
data_mean_violation_bfi |>
  filter(score_violation == "exclude") |>
  select(unique_id) |>
  kable() |>
  kable_classic(full_width = FALSE)
...
witin the study each participant completed an mplicit Association Test assessing implicit racial evaluations between African-American and European-American people. Block 1, 2, 5 are practice blocks and must be discarded for analysis calculations. 

First we check the dataset visually and we see that there are some variables that are not too practical to work with so we clean out the dataset. participant = unique_Id, block = block_number, X3 = block required responses, trial = trial responses, x 5 = trial accuracy x6 = trial reaction time (in ms). After we make sure that the trial_reaction time is still a numeric variable. Since there was the character description within the variable and it got removed, there is a possibility that it lead to conversion issues. 
...
data_iat_corrected <- data_raw_iat |>
  ...
  slice(-1) #excluding the first row since these represented the description we implied as variable names                above 

#making sure that trial_raction_time_in_ms is a numeric argument, as we have to calculate with it: 

data_iat_corrected$trial_reaction_time_in_ms <- as.numeric(data_iat_corrected$trial_reaction_time_in_ms)
...
1. Include a sanity check: check that all D scores are in the range -2 to +2. If not, revise your              implementation of the scoring code.
...
#No d_score violations for the dataset 
data_d_violation |>
...
2. Create an exclusion variable and set participants with incomplete trial level IAT data to “exclude”.        Specifically, IAT should have 120 trials on the critical test blocks (i.e., blocks 3, 4, 6 and 7). Trials
   on the other (practice) blocks can be discarded.
...
data_iat_incomplete <- data_iat_corrected |>
  ...
  ungroup() |>
  mutate(completed_trials = ifelse(trial_count == 120, "include", "exclude")) |>
  select(unique_id, completed_trials) %>%
  distinct()  # Only keep one row per ID as they have multiple rows due to several trials 



#some individuals do have uncompleted trials and must therefore be excluded  
data_iat_incomplete |>
  filter(completed_trials == "exclude") |>
  select(unique_id) |>
  kable() |>
  kable_classic(full_width = FALSE)
...
3. Create an exclusion variable for IAT performance: set participants with  >10% of the participants trials    are < 300ms, or if their accuracy is < than 75%. Only use trials from the critical test blocks when         computing these (i.e., blocks 3, 4, 6 and 7).


```{r}

data_iat_corrected_performance <- data_iat_corrected |>
  filter(block_number %in% c(3, 4, 6, 7)) |>
  mutate(low_reaction_time = ifelse(trial_reaction_time_in_ms < 300, TRUE, FALSE),
         low_accuracy = ifelse(trial_accuracy == "incorrect", TRUE, FALSE)) |>
  group_by(unique_id) |>
  summarize(proportion_fast_trials_iat = mean(low_reaction_time),
            proportion_low_accuracy_iat = mean(low_accuracy)) |>
  mutate(exclude_accuracy = ifelse(proportion_fast_trials_iat >= 0.10 | 
                                   proportion_low_accuracy_iat > 0.25,
                                   "exclude",
                                   "include"))

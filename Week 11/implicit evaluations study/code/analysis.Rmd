...
#The independent two-sample T-test serves to assess whether two groups differ with regard to a certain variable and is therefore permissible as a test value for the present hypothesis. The T-test indicates a significant difference between men and women (p= 0.0106). Similarly, the 95% confidence interval does not contain zero, which likewise underlines the significance of the differences in the self-report values between men and women. The present results can thus show that a significant difference is evident between the self-reported mean evaluations of men and women. 
...
#What does this test do exactly ? It 
kruskal_result <- kruskal.test(mean_evaluation ~ gender, data = data_processed_after_exclusions)

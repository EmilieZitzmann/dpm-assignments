...
#Calculate the mean amp score for women and men 
...
table_descriptives <- data_descriptives_amp %>% group_by(group = gender) %>% 
  summarise(Mean = mean(amp_score, na.rm = TRUE),
            SD = sd(amp_score, na.rm = TRUE)) %>% na.omit() %>% adorn_pct_formatting(digits = 2)
